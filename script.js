document.addEventListener('DOMContentLoaded', function () {
    ...
    const cancelarBtn = document.getElementById('cancelarBtn'); // Elemento do botão Cancelar
    ...
    function startCountdown() {
        ...
        const targetDate = new Date('December 31, 2025 23:59:59').getTime(); // Altere esta data
    ...
    // Lógica para fechar o modal ao clicar em Cancelar
    if (cancelarBtn) {
        cancelarBtn.addEventListener('click', () => {
            if (inscricaoModal) {
                inscricaoModal.style.display = 'none';
                console.log("Botão Cancelar clicado. Modal de inscrição fechado.");
            }
        });
    }
    ...
    if (inscricaoModal) {
        inscricaoModal.addEventListener('click', (e) => {
            if (e.target === inscricaoModal) {
                inscricaoModal.style.display = 'none';
                console.log("Modal de inscrição fechado (clique fora).");
            }
        });
    }
    ...
    if (inscricaoForm) {
        inscricaoForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            console.log("Formulário de inscrição submetido.");
            ...
            console.log("Coletando dados...", formData);

            try {
                console.log("Tentando enviar os dados para o servidor no endereço '/inscrever'...");
                ...
                if (!response.ok) {
                    ...
                    console.error("Resposta do servidor recebida:", errorText);
